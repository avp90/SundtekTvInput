...
public class ProgramsDB {
    ...
    public ProgramsDB(SundtekTvInputApp app) {
        if (DEBUG) Log.d(TAG, "create new ProgramsDB Instance");
        this.app = app;
    ...
    private void getPrograms(Context context, Channel channel, long startMs, long endMs) {
        ...
        for (Program program : programs) {
            ...
            if (DEBUG)
                Log.d(
                        TAG,
                        channelName + " - EventId: " + newEventId + " : " + (newProgram ? "new" : "replaced"));
        }
        ...
        Log.d(
                ...
                "Got "
                        + allProgramMap.size()
                        + " Programs for "
                        + channelProgramsMap.values().size()
    ...
    private ArrayList<Program> makeDummyProgram(Channel channel, long startMs, long endMs) {
        ...
        try {
            ...
            ipd.put(
                    PROG_IPD_EPG_EVENT_ID,
                    channel.getOriginalNetworkId() + "/" + channel.getOriginalNetworkId());
